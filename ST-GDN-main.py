...
import numpy as np
#import Attention
#import diffusionconv
from keras.optimizers import RMSprop, Adam
np.random.seed(1337)

X_train, Y_train, X_test, Y_test = flowdatacheck.X_train, flowdatacheck.Y_train, flowdatacheck.X_test, flowdatacheck.Y_test
...
def stresnet(c_conf=(3, 2, 10, 20), p_conf=(2, 2, 10, 20), t_conf=(1, 2, 10, 20), nb_residual_unit=3):
    '''
    ...
    for conf in [c_conf, p_conf, t_conf]:
        if conf is not None:
            ...
            if len_seq != 1:
                #temporal_block = Attention(input)
                #diffusion_block = diffusionconv(temporal_block)
...
def eval_threshold(y, pred_y, threshold=0.032):
    pickup_y = y[:, 0]
    dropoff_y = y[:, 1]
...
if __name__ == '__main__':
    ...
    print('Test mse (norm): %.6f rmse (real): %.6f' %
          (score, (score ** 0.5) * (307 - 0)))
    print("\nelapsed time (eval cont): %.3f seconds\n" % (time.time() - ts))

    # print('Training ------------')
    # stresnet.fit(X_train, Y_train, epochs=150, batch_size=32)

    print('\nTesting ------------')
    pred = stresnet.predict(X_test, batch_size=Y_test.shape[0])
    # Evaluate the model with the metrics we defined earlier
    rmse(Y_test, pred)
    mape(Y_test, pred)
    eval_threshold(Y_test, pred)